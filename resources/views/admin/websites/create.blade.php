...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <!-- Page Header -->
    <div class="mb-6">
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Add New Website</h1>
        <nav class="text-sm font-medium text-gray-500 dark:text-gray-400" aria-label="Breadcrumb">
            <ol class="list-none p-0 inline-flex space-x-2">
                ...
                <li><span>&gt;</span></li>
                <li class="text-gray-700 dark:text-gray-200" aria-current="page">Add Website</li>
            </ol>
        </nav>
    </div>
        ...
        <form method="POST" action="{{ route("admin.websites.store") }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old("name") }}" required placeholder="My Awesome Blog"
                ...
                <!-- User -->
                <div>
                    <label for="user_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Assign to User <span class="text-red-500">*</span></label>
                ...
                <div>
                    ...
                    <input type="text" name="niche" id="niche" value="{{ old("niche") }}" placeholder="e.g., Technology, Health, Finance"
                ...
                <div>
                    ...
                    <input type="text" name="website_type" id="website_type" value="{{ old("website_type") }}" placeholder="e.g., Blog, E-commerce, Portfolio"
                           class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-white">
                    @error("website_type")
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    <label for="license_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Assign License (Optional)</label>
                    <select name="license_id" id="license_id"
                        ...
                        <option value="" {{ old("license_id") ? "" : "selected" }}>No License</option>
                        @foreach($licenses as $license)
                            <option value="{{ $license->id }}" {{ old("license_id") == $license->id ? "selected" : "" }}>{{ $license->key }} ({{ $license->status }})</option>
                        @endforeach
                    </select>
                     <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Only active licenses might be shown here. A license can typically be assigned to only one website.</p>
                    @error("license_id")
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
